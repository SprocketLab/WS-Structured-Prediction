...
class Mallows:
    def __init__(self, rank_utils, theta):
        ...
        dist_probs = np.array(
            [
                (dist_counts[i] / Z_theta) * exp(-theta * i)
    ...
    def build_perm_dict(self, center_pi):
        ...
        for perm in perms:
            ...
            dist = int(
                self.rank_utils.kendall_tau_distance(perm_r, center_pi, normalize=False)
            )
    ...
    def find_eq_theta_to_p0(d, p_0):
        ...
        def jac(theta):
            ...
            return (log(A1_theta) - log_z_theta - log(p_0)) * (
                A1_jac_theta / A1_theta - mlw.d_log_partition(theta, d)
            )

        theta_0 = 0.01
        out = scipy.optimize.minimize_scalar(
            fun,
            bracket=(0.01, 10),
            bounds=(0.01, 10),
            tol=1e-50,
    ...
    def d_log_partition_1(theta, d):
        return (
            Mallows.log_partition_fun(theta + 1e-5, d)
            - Mallows.log_partition_fun(theta, d)
        ) / 1e-5
    ...
    def estimate_theta(d, mu_hat):
        ...
        def jac(theta):
            return -(-Mallows.d_log_partition(theta, d) - mu_hat) * (
                Mallows.d_d_log_partition(theta, d)
            )

        out = scipy.optimize.minimize(
            f, x0=[theta_0], jac=jac, bounds=[(0.1, 10.0)], tol=1e-18
        )
